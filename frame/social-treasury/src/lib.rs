...
use frame_support::traits::{
	Get, Imbalance, OnUnbalanced, ExistenceRequirement::{KeepAlive, AllowDeath},
	WithdrawReason
};
use sp_runtime::{Permill, Percent, RuntimeDebug, DispatchResult, traits::{
	Zero, StaticLookup, Saturating, Hash, BadOrigin
}};
...
type PositiveImbalanceOf<T> =
	pallet_mission_tokens::PositiveImbalance<T>;
type NegativeImbalanceOf<T> =
	pallet_mission_tokens::NegativeImbalance<T>;
...
impl<T: Trait> Module<T> {
	...
	fn insert_tip_and_check_closing(
		...
		if tip.tips.len() >= threshold && tip.closes.is_none() {
			tip.closes = Some(system::Module::<T>::block_number() + <T as Trait>::TipCountdown::get());
			true
		} else {
			false
		}
	...
	fn payout_tip(hash: T::Hash, tip: OpenTip<T::AccountId, BalanceOf<T>, T::BlockNumber, T::Hash, TokenId<T>>) {
		...
		Self::deposit_event(RawEvent::TipClosed(hash, tip.who, tip.mission_token_id, payout));
	}
			...
			v.retain(|&index| {
				...
				if let Some(p) = Self::proposals(index) {
					...
					if p.value <= budgets_remaining[id] {
						...
						let _ = <pallet_mission_tokens::Module<T>>::unreserve(&p.proposer, p.mission_token_id, p.bond);

						// provide the allocation.
						imbalances[id].subsume(<pallet_mission_tokens::Module<T>>::deposit_creating(&p.beneficiary, p.mission_token_id, p.value));

						Self::deposit_event(RawEvent::Awarded(index, p.mission_token_id, p.value, p.beneficiary));
						false
					} else {
						missed_any[id] = true;
						true
			...
			v.retain(|&index| {
				Bounties::<T>::mutate(index, |bounty| {
					...
					if let Some(bounty) = bounty {
                        let id: usize = (bounty.mission_token_id.unique_saturated_into() as usize).into();
						if bounty.value <= budgets_remaining[id] {
							...
							// return their deposit.
							let _ = <pallet_mission_tokens::Module<T>>::unreserve(&bounty.proposer, bounty.mission_token_id, bounty.bond);

							// fund the bounty account
							imbalances[id].subsume(<pallet_mission_tokens::Module<T>>::deposit_creating(&Self::bounty_account_id(index), bounty.mission_token_id, bounty.value));
	...
	fn create_bounty(
		...
		ensure!(description.len() <= <T as Trait>::MaximumReasonLength::get() as usize, Error::<T>::ReasonTooBig);
		ensure!(value >= <T as Trait>::BountyValueMinimum::get(), Error::<T>::InvalidValue);
		...
		let bounty = Bounty {
			proposer, value, fee: 0.into(), curator_deposit: 0.into(), bond, status: BountyStatus::Proposed, mission_token_id: token_id
		};
