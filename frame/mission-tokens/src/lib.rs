...
use sp_std::prelude::*;
use sp_std::{cmp, result, ops::BitOr, convert::Infallible};
...
use frame_support::{
	...
	traits::{
		...
		Imbalance, Get, ExistenceRequirement::AllowDeath,
	},
    Parameter,
};
use frame_system::{RefCount, ensure_signed, split_inner};
use sp_runtime::{DispatchError, RuntimeDebug,
    traits::{AtLeast32BitUnsigned, Bounded, CheckedAdd, CheckedSub, Member, StaticLookup, Saturating, Zero}
};
...
pub trait Trait: frame_system::Trait {
	...
	type AccountStore: StoredMap<(Self::MissionTokenId, Self::AccountId), AccountData<Self::Balance>>;
...
impl<T: Trait> Module<T> {
	...
	pub fn free_balance(who: impl sp_std::borrow::Borrow<T::AccountId>, token_id: T::MissionTokenId) -> T::Balance {
		Self::account(token_id, who.borrow()).free
	}

	/// Get the reserved balance of an account.
	pub fn reserved_balance(who: impl sp_std::borrow::Borrow<T::AccountId>, token_id: T::MissionTokenId) -> T::Balance {
	...
	pub fn do_transfer(
		...
		Self::try_mutate_account(dest, token_id, |to_account, _| -> DispatchResult {
			Self::try_mutate_account(transactor, token_id, |from_account, _| -> DispatchResult {
				...
				ensure!(allow_death || from_account.free >= ed, Error::<T>::KeepAlive);
		...
		// Emit transfer event.
		Self::deposit_event(RawEvent::Transfer(transactor.clone(), token_id, dest.clone(), value));
	...
	pub fn reserve(who: &T::AccountId, token_id: T::MissionTokenId, value: T::Balance) -> DispatchResult {
		...
		Self::try_mutate_account(who, token_id, |account, _| -> DispatchResult {
			...
			Self::ensure_can_withdraw(&who, token_id, value.clone(), WithdrawReason::Reserve.into(), account.free)
		})?;
	...
	pub fn unreserve(who: &T::AccountId, token_id: T::MissionTokenId, value: T::Balance) -> T::Balance {
	...
	fn withdraw(
		...
		Self::try_mutate_account(who, token_id, |account, _|
			-> Result<NegativeImbalance<T>, DispatchError>
		{
	...
	pub fn deposit_creating(
		...
		Self::try_mutate_account(who, token_id, |account, is_new| -> Result<PositiveImbalance<T>, PositiveImbalance<T>> {
			...
			account.free = account.free.checked_add(&value).ok_or_else(|| PositiveImbalance::zero())?;
	...
	/// total issuance.
	pub fn pair(token_id: T::MissionTokenId, amount: T::Balance) -> (PositiveImbalance<T>, NegativeImbalance<T>) {
		(Self::burn(token_id, amount.clone()), Self::issue(token_id, amount))
	}
...
// of the inner member.
mod imbalances {
    use super::{result, Imbalance, Trait, Zero, Saturating, TryDrop};
	use sp_std::mem;
...
impl<T: Trait> StoredMap<(T::MissionTokenId, T::AccountId), <T as Trait>::AccountData> for Module<T> {
	...
	fn mutate<R>(k: &(T::MissionTokenId, T::AccountId), f: impl FnOnce(&mut <T as Trait>::AccountData) -> R) -> R {
		let existed = SystemAccount::<T>::contains_key(k);
	...
	fn mutate_exists<R>(k: &(T::MissionTokenId, T::AccountId), f: impl FnOnce(&mut Option<<T as Trait>::AccountData>) -> R) -> R {
		Self::try_mutate_exists(k, |x| -> Result<R, Infallible> { Ok(f(x)) }).expect("Infallible; qed")
	}
	fn try_mutate_exists<R, E>(k: &(T::MissionTokenId, T::AccountId), f: impl FnOnce(&mut Option<<T as Trait>::AccountData>) -> Result<R, E>) -> Result<R, E> {
